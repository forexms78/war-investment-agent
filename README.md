...
```
PO (사용자)
  └─ PM (Claude) ── 요구사항 분석·작업 분배·결과 통합
       ├─ Backend Dev (Claude) ── FastAPI·서비스 레이어·API 설계
       └─ Frontend Dev (Claude) ── Next.js·컴포넌트·UI 시스템
```
...
**오케스트레이션 효과**
- 기획 → 백엔드 서비스 → 프론트 컴포넌트 → 배포까지 단일 세션 내 완성
- 기존 순차 개발 대비 컨텍스트 유실 없이 일관된 아키텍처 유지
- 에이전트 역할 분리로 백엔드·프론트 충돌 없이 병렬 작업
