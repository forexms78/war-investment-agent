...
| 버전 | 날짜 | 내용 |
|------|------|------|
| v3.14 | 2026-05-28 | **My Lab 리밸런싱 적용 뷰 신규 — 6번모드(투자 전문가) 분석 연동.** `stock78/analyses/YYYY-MM-DD-rebalance.json`(수동 push, git 형상관리)을 읽는 `GET /mylab/rebalance` 엔드포인트 추가(`mylab.py` `get_latest_rebalance()` — 최신 *-rebalance.json 1개 파싱). 프론트 My Lab에 "적용" 토글 추가(현재/적용/히스토리 3분기), `RebalanceView` 신규 — 테마 비중 적용 전→후 바(`ThemeAllocationBar`), 종목별 액션 오버레이(`ActionList` — 매도/축소는 빨강 빗금, 신규 매수는 초록 테두리 추가행), 보유종목 비중 도넛(`HoldingDonut`, recharts). 분석 없을 때 안내 표시. 기존 현재/히스토리 뷰 무변경 |
| v3.13 | 2026-05-28 | **장 상태 표시기 보강 — 한국 프리마켓 구분 + 마감/다음 개장 동시 표시.** 한국 08:30~08:40을 "프리마켓"으로 별도 표시(동시호가 08:40~09:00로 분리, 우선순위 적용). 호버 팝오버에서 현재 장이 **몇 시에 마감**되는지와 **다음 장이 무엇이고 몇 시에 개장**하는지(KST 환산·남은 시간 포함)를 함께 표기. `nextSession()`이 현재 시각 이후 첫 세션을 찾아 라벨·개장시각 산출, 주말/익영업일 롤오버 처리 |
| v3.12 | 2026-05-28 | **실시간 장 상태 표시기로 전환.** v3.11의 "? 도움말" 정적 툴팁을 제거하고, 헤더에 현재 한국시간 기준 한국·미국 장 상태를 실시간 표시(`components/MarketStatus.tsx`). 한국(동시호가/정규장/장후시간외/시간외단일가)·미국(프리/정규/애프터)을 `Intl` timeZone(Asia/Seoul·America/New_York)으로 현지 벽시계 판정 → 서머타임(EDT/EST) 자동 반영, 별도 분기 불필요. 상태 점(정규=초록 펄스/시간외=파랑/마감=회색) + 라벨, 20초 간격 갱신. 호버 시 마감/개장 시각(KST 환산)·남은 시간·현재 EDT/EST·활성 세션 강조 스케줄표 표시. 주말 자동 마감 처리(공휴일 미반영) |
| v3.11 | 2026-05-28 | **장 운영 시간 도움말 추가.** 대시보드 헤더 다크/라이트 토글 옆에 "? 도움말" 버튼 신규(`components/MarketHoursHelp.tsx`). 한국(KRX) 장전 동시호가·프리마켓·정규장·애프터·시간외 단일가 + 미국(NASDAQ/NYSE) 서머타임 적용/미적용 한국시간 환산표 + 용어(프장/본장/애프터/동시호가) + 거래량·변동성·체결가 팁을 정리한 팝오버. 클릭(탭) 토글 방식 — 외부 클릭·ESC·스크롤 시 자동 닫힘, 모바일 대응(`maxWidth: calc(100vw-24px)`), CSS 변수 기반으로 라이트/다크 테마 자동 적응 |
