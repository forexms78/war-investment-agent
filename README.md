...
| 버전 | 날짜 | 내용 |
|------|------|------|
| v3.4 | 2026-05-26 | **My Lab 탭 신규** — GitHub `forexms78/stock78` private repo 연동. `backend/services/mylab.py` 신규(GitHub API로 `analyses/` 폴더 MD 파일 목록·내용 fetch, `MYLAB_PASSWORD` 서버사이드 인증, 경로 순회 방지). `GET /mylab/analyses`, `GET /mylab/analyses/{filename}`, `POST /mylab/auth` 엔드포인트 추가. 프론트 `MyLabSection.tsx` 신규 — 잠금 게이트(비밀번호 인증·localStorage 상태 유지), 분석 카드 그리드, 마크다운 렌더링 인라인 파서(# 헤더·**굵게**·목록·코드블록·링크), 내용 모달(ESC 닫기). `GITHUB_TOKEN`·`MYLAB_PASSWORD` 환경변수 필요 |
